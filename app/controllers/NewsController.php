...
class NewsController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('id', 	'title', 	'content', 	'image', 	'category', 	'created_at', 	'updated_at');
		...
		$records = $db->get('news', $limit, $fields);
		$data = new stdClass;
		...
		if(!empty($_FILES['file'])){
			...
			$file_path = $_FILES['file']['tmp_name'];
				if(!empty($file_path)){
					$db = $this->GetModel();
					if($ext == 'csv'){
						$options = array('table' => 'news', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
						$data = $db->loadCsvData( $file_path , $options , false );
					}
					else{
						$data = $db->loadJsonData( $file_path, 'news' , false );
					}
					if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			if( empty($this->view->page_error) ){
				$db = $this->GetModel();
				$rec_id = $db->insert( 'news' , $modeldata );
				if(!empty($rec_id)){
					set_flash_msg('','');
