...
class RolesController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('id', 	'slug', 	'name', 	'permissions', 	'created_at', 	'updated_at');
		...
		$records = $db->get('roles', $limit, $fields);
		$data = new stdClass;
		...
		$this->view->render('roles/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			$file_path = $_FILES['file']['tmp_name'];
				if(!empty($file_path)){
					$db = $this->GetModel();
					if($ext == 'csv'){
						$options = array('table' => 'roles', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
						$data = $db->loadCsvData( $file_path , $options , false );
					}
					else{
						$data = $db->loadJsonData( $file_path, 'roles' , false );
					}
					if($db->getLastError()){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'roles/list');
	...
	function view( $rec_id = null , $value = null){
		...
		$record = $db->getOne( 'roles', $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Roles";
			$this->view->render('roles/view.php' , $record ,'main_layout.php');
		}
		else{
			if($db->getLastError()){
				$this->view->page_error = $db->getLastError();
	...
	function add(){
		if(is_post_request()){
			...
			if( empty($this->view->page_error) ){
				$db = $this->GetModel();
				$rec_id = $db->insert( 'roles' , $modeldata );
				if(!empty($rec_id)){
					set_flash_msg('','');
					redirect_to_page("roles");
					return;
				}
				else{
					if($db->getLastError()){
		...
		$this->view->page_title ="Add New Roles";
	...
	function edit($rec_id=null){
		...
		if(is_post_request()){
			...
			if(empty($this->view->page_error)){
				$db->where('id' , $rec_id);
				$bool = $db->update('roles',$modeldata);
				if($bool){
					set_flash_msg('','');
					redirect_to_page("roles");
					return;
		...
		$fields = array('id','slug','name','permissions','created_at','updated_at');
		$db->where('id' , $rec_id);
		$data = $db->getOne('roles',$fields);
		$this->view->page_title ="Edit  Roles";
		if(!empty($data)){
			$this->view->render('roles/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
				$this->view->page_error[] = $db->getLastError();
