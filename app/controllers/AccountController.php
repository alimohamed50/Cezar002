...
class AccountController extends SecureController{
	...
	function edit(){
		...
		if(is_post_request()){
			...
			if(empty($this->view->page_error)){
				$db->where('id' , USER_ID);
				$bool = $db->update('users2',$modeldata);
				if($bool){
				$db->where ('id', USER_ID);
				$user = $db->getOne('users2' , '*');
		...
		$fields = array('id','user_name','photo','user_role','date_registered');
		$db->where('id' , USER_ID);
		$data = $db->getOne('users2',$fields);
	...
	function change_email(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$db->where ("id", USER_ID);
			$result = $db->update('users2', array('email' => $email ));
			if($result){
